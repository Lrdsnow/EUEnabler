...
def retrieve_restore_files(data, eligibility_data=None, config_data=None):
    ...
    if data == False:
        files = [
            FileToRestore(contents=b'', restore_path="/var/db/os_eligibility/", restore_name="eligibility.plist")
        ] + [
            FileToRestore(contents=b'', restore_path=path, restore_name="Config.plist") for path in restore_paths
        ]
        return files
    else:
        files = [
